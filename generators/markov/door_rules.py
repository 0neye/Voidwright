...
def load_vanilla_part_geometry() -> Dict[str, VanillaPartGeometry]:
    with VANILLA_PARTS_PATH.open() as fh:
    ...
    for part in payload.get("parts", []):
        ...
        rotations: Dict[int, RotationGeometry] = {}
        for key, rot in per_rotation.items():
            rotation = int(rot.get("rotation_index", key))
            size = rot.get("size") or [0, 0]
            footprint_tiles = frozenset(tuple(map(int, cell)) for cell in rot.get("footprint_tiles", []))
            unblocked_tiles = frozenset(tuple(map(int, cell)) for cell in rot.get("unblocked_footprint_tiles", []))
            blocked_travel_cells = frozenset(tuple(map(int, cell)) for cell in rot.get("blocked_travel_cells", []))
            allowed_door_locations = tuple(tuple(map(int, cell)) for cell in rot.get("allowed_door_locations", []))
            rotations[rotation] = RotationGeometry(
                ...
                blocked_travel_cells=blocked_travel_cells,
                allowed_door_locations=allowed_door_locations,
            )
...
ORIENTATION_DELTAS = {0: (0, 1), 1: (1, 0)}
...
def infer_meta(part_id: str, rotation: int) -> Tuple[PartMeta, bool]:
    vanilla = load_vanilla_part_geometry().get(part_id)
    if vanilla is not None:
        rot = vanilla.rotations.get(rotation) or vanilla.rotations.get(rotation % 4) or next(iter(vanilla.rotations.values()))
        ...
        return PartMeta(
            ...
            unblocked_tiles=rot.unblocked_tiles,
            blocked_travel_cells=rot.blocked_travel_cells,
            allowed_door_locations=rot.allowed_door_locations,
        ), False
    ...
    return PartMeta(width, height, traversable, note, footprint_tiles, footprint_tiles if traversable else frozenset(), frozenset(), ()), True
...
def resolve_observation_from_cells(cell_to_parts: dict, cell: Coord, orientation: int) -> Optional[PlacementObservation]:
    delta = ORIENTATION_DELTAS.get(orientation)
    if delta is None:
        return None
