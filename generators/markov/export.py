...
from ship_parser.cosmoteer_ship_encoder import write_ship_png, create_ship_png_bytes
...
def roundtrip_validate(generated_json: dict) -> dict:
    ...
    import io as _io
    from ship_parser.cosmoteer_ship_parser import parse_ship_png as _parse
    from pathlib import Path as _Path

    # write to a temp bytes buffer and parse
    tmp = _io.BytesIO(png_bytes)

    class _BytesPath:
        """Minimal shim so parse_ship_png can accept bytes."""
        def __init__(self, data: bytes):
            self._data = data
        def read_bytes(self) -> bytes:
            return self._data

    from ship_parser.cosmoteer_ship_parser import _extract_embedded_payload, _decode_ship_data
    try:
