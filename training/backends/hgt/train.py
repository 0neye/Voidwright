...
def apply_overclock_mask(
    ...
    """Zero out the overclocked flag (``x[:, 6]``) for a random fraction of parts.

    Returns
    -------
    data:
        Modified batch with overclocked features zeroed for masked nodes.
    mask_indices:
    ...
    data["part"].x[mask_indices, 6] = 0.0
    return data, mask_indices, true_labels
...
def eval_epoch(
    ...
    *,
    ...
    """Run one evaluation pass; returns ``{"loss", "acc", "top5_acc"}``."""
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_top5 = 0
    ...
    for batch in loader:
        ...
        if overclock_mask_rate > 0.0:
            batch, _, _ = apply_overclock_mask(batch, overclock_mask_rate)
        if door_mask_rate > 0.0:
            batch, _, _ = apply_edge_mask(batch, DOOR_EDGE_KEY, door_mask_rate)
        if virtual_edge_mask_rate > 0.0:
            for ek in _VIRTUAL_EDGE_TYPES:
                batch, _, _ = apply_edge_mask(batch, ek, virtual_edge_mask_rate)
...
def load_checkpoint(path: Path, model: "ShipHGT", optimizer: "torch.optim.Optimizer | None" = None) -> dict:
