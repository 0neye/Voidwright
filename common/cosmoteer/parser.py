...
def _iter_png_chunks(raw: bytes) -> Iterator[tuple[bytes, bytes]]:
    """Yield `(chunk_type, chunk_data)` pairs from a raw PNG byte string."""
...
def _paeth_predictor(left: int, up: int, upper_left: int) -> int:
    """Return the PNG Paeth predictor value for one byte position."""
...
def _decode_png_rgb_bytes(raw: bytes) -> bytes:
    """Return decoded RGB channel bytes for a PNG image."""
...
def _read_embedded_byte(rgb_bytes: bytes, byte_offset: int) -> int:
    """Read one embedded byte from successive RGB least-significant bits."""
...
def _extract_embedded_payload(png_bytes: bytes) -> bytes:
    """Extract the compressed Cosmoteer payload from PNG bytes."""
...
def _read_varint(stream: BinaryIO) -> int:
    """Read a Cosmoteer integer encoded in the custom varint format."""
...
def _read_length_prefixed_string(stream: BinaryIO) -> str:
    """Read a LEB128-length-prefixed latin1 string."""
...
def _convert_value_bytes(key: str, value: bytes) -> Any:
    """Decode a primitive value payload using key-specific heuristics."""
...
def _try_decode_embedded_string(value: bytes) -> str | None:
    """Return the decoded string when *value* looks like an embedded string."""
...
def _normalize_for_json(value: Any) -> Any:
    """Normalize decoded object values into JSON-serializable Python types."""
...
def _decode_object(stream: BinaryIO) -> Any:
    """Decode one Cosmoteer object node from the binary stream."""
...
def _decode_ship_data(payload: bytes) -> Any:
    """Decode the compressed embedded ship payload into a Python object."""
